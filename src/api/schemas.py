...
class CreditApplication(BaseModel):
    ...
    score_bancario: int = Field(..., ge=300, le=850, description="Score bancario (300-850)")
    
    # Información laboral
    antiguedad_empleo: int = Field(..., ge=0, description="Antigüedad en meses en el empleo actual")
    tipo_contrato: str = Field(..., description="Tipo de contrato laboral")
    
    # Historial crediticio
    num_creditos_previos: int = Field(..., ge=0, description="Número de créditos previos")
    max_dias_mora: int = Field(..., ge=0, description="Máximo días de mora en créditos anteriores")
    creditos_problematicos: int = Field(..., ge=0, description="Créditos con problemas de pago")
    
    # Información adicional
    tipo_vivienda: str = Field(..., description="Tipo de vivienda")
    antiguedad_residencia: int = Field(..., ge=0, description="Años en la residencia actual")
    ...
    def validate_expenses(cls, v, values):
        ...
        if 'ingreso_mensual' in values and v > values['ingreso_mensual'] * 0.9:
            raise ValueError('Los gastos mensuales no pueden exceder el 90% del ingreso')
        return v
...
class PredictionResponse(BaseModel):
    ...
    probability_default: float = Field(..., ge=0, le=1, description="Probabilidad de default")
    risk_score: int = Field(..., ge=300, le=850, description="Score de riesgo (300-850)")
    risk_category: str = Field(..., description="Categoría de riesgo")
    features_used: List[str] = Field(..., description="Features utilizadas en la predicción")
    message: str = Field(..., description="Mensaje descriptivo")


class BatchPredictionRequest(BaseModel):
    """Solicitud de predicciones por lotes."""
    applications: List[CreditApplication] = Field(..., description="Lista de solicitudes")
