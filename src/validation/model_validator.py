...
class ModelValidator:
    ...
    def validate_on_new_data(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if missing_features:
            ...
            for feature in missing_features:
                ...
                if '_' in feature and any(base in feature for base in ['estado', 'nivel', 'tipo']):
        ...
        if has_target and y_true is not None:
            ...
            metrics = {
                ...
                'roc_auc': float(roc_auc_score(y_true, y_proba)) if y_proba is not None else 0.5,
                'sample_size': len(df)
            }
    ...
    def calculate_drift_metrics(self, reference_data: pd.DataFrame, 
        ...
        if self.target_name in reference_data.columns and self.target_name in current_data.columns:
            ref_target_dist = reference_data[self.target_name].value_counts(normalize=True)
            curr_target_dist = current_data[self.target_name].value_counts(normalize=True)
        ...
        logger.info(f"Drift detectado en {len(drifted_features)}/{len(numeric_cols)} features")
        ...
        with open(report_path, 'w') as f:
            ...
            if validation_results.get('has_target', False):
                ...
                if 'confusion_matrix' in metrics:
                    f.write(f"\n🎯 MATRIZ DE CONFUSIÓN:\n")
                    f.write(f"  TN: {metrics['true_negatives']} | FP: {metrics['false_positives']}\n")
                    f.write(f"  FN: {metrics['false_negatives']} | TP: {metrics['true_positives']}\n")
            ...
            if predictions_df is not None and 'prediction' in predictions_df.columns:
                ...
                for value, count in pred_dist.items():
                    f.write(f"  Clase {value}: {count} ({count/len(predictions_df)*100:.1f}%)\n")
                ...
                if 'risk_category' in predictions_df.columns:
                    ...
                    for category, count in risk_dist.items():
                        f.write(f"  {category}: {count} ({count/len(predictions_df)*100:.1f}%)\n")
            ...
            if original_metrics and validation_results.get('has_target', False):
                ...
                if 'test' in original_metrics:
                    f.write(f"  ROC AUC Original: {original_metrics['test'].get('roc_auc', 'N/A'):.4f}\n")
                    f.write(f"  ROC AUC Validación: {validation_results['metrics'].get('roc_auc', 'N/A'):.4f}\n")
                    diff = validation_results['metrics'].get('roc_auc', 0) - original_metrics['test'].get('roc_auc', 0)
                    f.write(f"  Diferencia: {diff:+.4f}\n")
        
        logger.info(f"Reporte generado en: {report_path}")
        return str(report_path)
    
    def create_validation_plots(self, validation_results: Dict[str, Any], 
        ...
        if 'probability_default' in predictions_df.columns:
            ...
            sns.histplot(data=predictions_df, x='probability_default', bins=30, kde=True)
        ...
        if (validation_results.get('has_target', False) and 
            'probability_default' in predictions_df.columns and 
            self.target_name in predictions_df.columns):
...
def validate_model_cli():
    ...
    parser.add_argument('--model', type=str, required=True,
                       help='Ruta al modelo guardado (.pkl)')
    parser.add_argument('--data', type=str, required=True,
                       help='Archivo con datos para validación')
    ...
    # Generar reporte
    report_path = validator.generate_validation_report(validation_results, args.output_dir)
