...
class GeneradorDatosCrediticios:
    ...
    def generar_datos_demograficos(self) -> pd.DataFrame:
        ...
        for i in range(self.n_clientes):
            ...
            cliente = {
                ...
                'tipo_vivienda': random.choice(['propia', 'arrendada', 'familiar', 'hipotecada']),
                'ciudad': self.faker.city(),
                'antiguedad_residencia': random.randint(0, min(edad-18, 30)),
                'telefono': self.faker.phone_number(),
                'email': self.faker.email(),
                'fecha_registro': self.faker.date_between(start_date='-10y', end_date='today')
            }
    ...
    def generar_datos_financieros(self) -> pd.DataFrame:
        ...
        for i in range(self.n_clientes):
            ...
            datos = {
                ...
                'empresa_tamano': random.choice(['pequena', 'mediana', 'grande']) if tiene_empleo else 'N/A',
                'otros_ingresos': round(random.uniform(0, 500), 2),
                'gastos_mensuales': round(ingreso_mensual * random.uniform(0.3, 0.7), 2),
    ...
    def _asignar_fuente_ingreso(self, tiene_empleo: bool) -> str:
        ...
        if tiene_empleo:
            ...
            return random.choice(['pension', 'subsidio', 'negocio_propio', 'inversiones'])
    ...
    def generar_historial_crediticio(self) -> pd.DataFrame:
        ...
        for i in range(self.n_clientes):
            ...
            for j in range(n_creditos):
                ...
                # Fechas realistas
                fecha_inicio = self.faker.date_between(
                    start_date='-5y', 
                    end_date='-6m'
                )
                ...
                historial.append({
                    ...
                    'fecha_fin': fecha_fin if fecha_fin < datetime.now().date() else None,
                    'estado': estado,
                    'dias_mora': dias_mora,
                    'tipo_credito': random.choice(['personal', 'automotriz', 'hipotecario', 'tarjeta']),
                    'institucion': random.choice(['BancoA', 'BancoB', 'FinancieraC', 'CooperativaD'])
                })
    ...
    def generar_target_variable(self, df_demografico: pd.DataFrame, 
                               df_financiero: pd.DataFrame) -> pd.DataFrame:
        ...
        for i in range(self.n_clientes):
            ...
            demografico = df_demografico[df_demografico['cliente_id'] == cliente_id].iloc[0]
            financiero = df_financiero[df_financiero['cliente_id'] == cliente_id].iloc[0]
    ...
    def _calcular_score_riesgo(self, demografico: pd.Series, financiero: pd.Series) -> float:
    ...
    def generar_todos_datos(self, output_dir: str = 'data/raw'):
        ...
        df_demografico.to_csv(f'{output_dir}/clientes_demograficos_{fecha_str}.csv', index=False)
        df_financiero.to_csv(f'{output_dir}/clientes_financieros_{fecha_str}.csv', index=False)
        df_historial.to_csv(f'{output_dir}/historial_crediticio_{fecha_str}.csv', index=False)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Generador de datos sintéticos para riesgo crediticio')
    parser.add_argument('--clientes', type=int, default=1000, help='Número de clientes a generar')
    parser.add_argument('--output', type=str, default='data/raw', help='Directorio de salida')
