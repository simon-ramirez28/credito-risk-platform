...
class CreditFeatureEngineer:
    ...
    def _create_financial_risk_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if all(col in df.columns for col in ['total_adeudado', 'ingreso_anual']):
            df['deuda_ingreso_anual_ratio'] = np.where(
                df['ingreso_anual'] > 0,
                df['total_adeudado'] / df['ingreso_anual'],
                0
            )
        ...
        if all(col in df.columns for col in ['ahorros', 'gastos_mensuales']):
            df['meses_liquidez'] = np.where(
                df['gastos_mensuales'] > 0,
                df['ahorros'] / df['gastos_mensuales'],
                0
            )
    ...
    def _create_behavioral_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if all(col in df.columns for col in ['num_creditos_previos', 'edad']):
            ...
            df['intensidad_crediticia'] = np.where(
                df['edad'] > 18,
                df['num_creditos_previos'] / (df['edad'] - 18),
                0
            )
    ...
    def _create_interaction_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if all(col in df.columns for col in ['score_bancario', 'max_dias_mora']):
            df['score_ajustado_mora'] = df['score_bancario'] - (df['max_dias_mora'] * 0.1)
    ...
    def _create_grouped_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'ciudad' in df.columns and len(df['ciudad'].unique()) > 1:
            ...
            city_stats = df.groupby('ciudad').agg({
                'ingreso_mensual': ['mean', 'std'],
                'default': 'mean'
            }).round(2)
            
            city_stats.columns = ['ingreso_promedio_ciudad', 'ingreso_std_ciudad', 
                                  'default_rate_ciudad']
        ...
        if 'nivel_educacion' in df.columns:
            edu_stats = df.groupby('nivel_educacion').agg({
    ...
    def _create_composite_score_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if all(col in df.columns for col in ['ingreso_mensual', 'gastos_mensuales', 'ahorros']):
            capacidad_score = np.where(
                df['gastos_mensuales'] > 0,
                (df['ingreso_mensual'] - df['gastos_mensuales']) / df['gastos_mensuales'],
                0
            )
            # Normalizar a 0-1
        ...
        if all(col in df.columns for col in ['max_dias_mora', 'creditos_problematicos']):
            historial_score = 1 - (
        ...
        if all(col in df.columns for col in ['antiguedad_empleo', 'antiguedad_residencia']):
            estabilidad_score = (
                (np.clip(df['antiguedad_empleo'], 0, 60) / 60) * 0.6 +
                (np.clip(df['antiguedad_residencia'], 0, 30) / 30) * 0.4
            )
    ...
    def get_feature_importance(self, df: pd.DataFrame, target_col: str = 'default') -> pd.DataFrame:
        """
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Feature engineering para datos crediticios')
    parser.add_argument('--input', type=str, required=True, help='Archivo de entrada (CSV o Parquet)')
    parser.add_argument('--output', type=str, default='data/features/features_engineered.parquet', 
                       help='Archivo de salida')
    parser.add_argument('--target', type=str, default='default', help='Columna objetivo')
