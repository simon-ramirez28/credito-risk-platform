...
class PipelineETLCrediticio:
    ...
    def extract(
        self, archivos: Optional[Dict[str, str]] = None
    ...
    def transform(self, datos: Dict[str, pd.DataFrame]) -> pd.DataFrame:
        ...
        df_clientes = pd.merge(
            datos["demografico"], datos["financiero"], on="cliente_id", how="inner"
        )

        # 2. Agregar target
        logger.info("  Añadiendo variable target...")
        df_clientes = pd.merge(
            df_clientes, datos["target"], on="cliente_id", how="left"
        )
        ...
        historial_agg = (
            df_historial.groupby("cliente_id")
            .agg(
                {
        ...
        historial_agg.columns = [
            ...
            "promedio_dias_mora",
            "total_dias_mora",
            "creditos_problematicos",
            "tasa_interes_promedio",
        ]

        # 4. Unir con datos de clientes
        logger.info("  Uniendo historial crediticio...")
        df_completo = pd.merge(df_clientes, historial_agg, on="cliente_id", how="left")
        ...
        for col in columnas_numericas:
            if df_transformado[col].isnull().any():
                ...
                if "credito" in col.lower() or "mora" in col.lower():
                    ...
                    df_transformado[col] = df_transformado[col].fillna(
                        df_transformado[col].median()
                    )

        # 2. Codificar variables categóricas básicas
        if "genero" in df_transformado.columns:
            df_transformado["genero_encoded"] = df_transformado["genero"].map(
                {"M": 0, "F": 1}
            )

        if "estado_civil" in df_transformado.columns:
            # One-hot encoding básico
            estados_dummies = pd.get_dummies(
                df_transformado["estado_civil"], prefix="estado"
            )
    ...
    def _crear_features_basicas(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "ingreso_mensual" in df_features.columns:
            ...
            if "gastos_mensuales" in df_features.columns:
                df_features["ahorro_mensual"] = (
                    df_features["ingreso_mensual"] - df_features["gastos_mensuales"]
                )
        ...
        if "antiguedad_empleo" in df_features.columns:
            df_features["empleo_estable"] = (
                df_features["antiguedad_empleo"] >= 12
            ).astype(int)

        if "antiguedad_residencia" in df_features.columns:
            df_features["residencia_estable"] = (
                df_features["antiguedad_residencia"] >= 2
            ).astype(int)

        # 3. Features de riesgo crediticio
        if "num_creditos_previos" in df_features.columns:
            df_features["tiene_historial"] = (
                df_features["num_creditos_previos"] > 0
            ).astype(int)
        ...
        if "creditos_problematicos" in df_features.columns:
            df_features["tiene_problemas"] = (
                df_features["creditos_problematicos"] > 0
            ).astype(int)
    ...
    def load(self, df: pd.DataFrame, formato: str = "parquet") -> Dict[str, Any]:
        ...
        metadata = {
            ...
            "default_rate": (
                float(df["default"].mean()) if "default" in df.columns else None
            ),
        }
...
def main():
    ...
    parser.add_argument(
        "--input", type=str, default="data/raw", help="Directorio de entrada"
    )
    parser.add_argument(
        "--output", type=str, default="data/processed", help="Directorio de salida"
    )
    parser.add_argument(
        "--format",
        type=str,
