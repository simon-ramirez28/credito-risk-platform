...
class CreditRiskModel:
    ...
    def handle_imbalance(self, X_train: pd.DataFrame, y_train: pd.Series) -> Tuple:
        ...
        if class_counts.min() / class_counts.max() < 0.3:
            ...
            logger.info(f"Distribución después de SMOTE: {dict(pd.Series(y_train_balanced).value_counts())}")
    ...
    def train(self, X_train: pd.DataFrame, y_train: pd.Series, 
        ...
        train_metrics = self._calculate_metrics(y_train_balanced, y_train_pred, y_train_proba)
        self.metrics['train'] = train_metrics
        
        logger.info(f"Entrenamiento completado. Train ROC AUC: {train_metrics['roc_auc']:.4f}")
        return self.metrics
    ...
    def create_model_report(self) -> str:
        ...
        if not feature_importance.empty:
            ...
            for i, row in feature_importance.head(10).iterrows():
                report_lines.append(f"  {i+1:2d}. {row['feature'][:40]:40s} : {row['importance']:.4f}")
...
def train_model_from_cli():
    ...
    parser.add_argument('--data', type=str, required=True, 
                       help='Archivo con features (CSV o Parquet)')
